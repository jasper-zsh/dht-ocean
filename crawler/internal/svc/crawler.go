...
func NewCrawler(svcCtx *ServiceContext) (*Crawler, error) {
	...
	c := &Crawler{
		...
		nodeID:         nodeID,
		tm:             &dht.TransactionManager{},
		rawPackets:     make(chan *dht.Packet, svcCtx.Config.PacketQueueSize),
		decodedPackets: make(chan *dht.Packet, svcCtx.Config.PacketQueueSize),
		outPackets:     make(chan *outPacket, svcCtx.Config.PacketQueueSize),
